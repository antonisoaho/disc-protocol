export const commonSv = {
  shell: {
    loadingSession: 'Laddar session…',
    logoAlt: 'Disc Golf Social — Disco',
    appTitle: 'Disc Golf Social',
    signInPrompt: 'Logga in för att fortsätta.',
    nav: {
      home: 'Hem',
      courses: 'Banor',
      primaryAria: 'Primär navigering',
    },
    bottomNav: {
      aria: 'Huvudsektioner',
      home: 'Hem',
      players: 'Spelare',
    },
    signOut: 'Logga ut',
    signOutError: 'Kunde inte logga ut. Försök igen.',
    profile: 'Profil',
    offlineBanner:
      'Du verkar vara offline. Ändringar köas lokalt och synkas när du är online igen.',
    userProfileSyncFailed:
      'Vi kunde inte spara din spelarprofil i molnet. Vissa funktioner behöver den profilen.',
    retryUserProfileSync: 'Försök synka igen',
  },
  profile: {
    title: 'Profil',
    displayName: 'Visningsnamn',
    currentDisplayName: 'Nuvarande visningsnamn: {{displayName}}',
    actions: {
      save: 'Spara',
      saving: 'Sparar…',
    },
    messages: {
      displayNameSaved: 'Visningsnamnet uppdaterades.',
      displayNameSavedLocalOnly:
        'Visningsnamnet sparades i appen. Din inloggningsleverantör tillåter kanske inte att kontots visningsnamn uppdateras.',
    },
    errors: {
      displayNameRequired: 'Visningsnamn krävs.',
      displayNameTooLong: 'Visningsnamn måste vara högst {{max}} tecken.',
      displayNameUpdateFailed: 'Kunde inte uppdatera visningsnamnet.',
    },
    teams: {
      title: 'Scramble-lag',
      hint: 'Spara laggrupper här. De visas när du startar en scramble-runda och kan användas i guiden för ny runda.',
      you: 'Du',
      teamName: 'Lagnamn',
      members: 'Medlemmar',
      addTeam: 'Lägg till lag',
      editTeam: 'Redigera',
      doneEditing: 'Klar',
      removeTeam: 'Ta bort lag',
      defaultName: 'Lag {{number}}',
      unassigned_one: '{{count}} spelare saknar lag',
      unassigned_other: '{{count}} spelare saknar lag',
      noMembers: 'Inga medlemmar än',
      clubTeamsTitle: 'Dina klubblag',
      clubTeamsHint: 'Lag som en admin hanterar och som du tillhör (skrivskyddade här).',
      clubTeamsAria: 'Klubblag du tillhör',
      clubTeamLine: '{{teamName}}: {{members}}',
      saved: 'Lag sparade.',
      saveFailed: 'Kunde inte spara lag.',
    },
  },
  auth: {
    title: {
      signIn: 'Logga in',
      createAccount: 'Skapa konto',
    },
    fields: {
      email: 'E-post',
      password: 'Lösenord',
    },
    actions: {
      working: 'Arbetar…',
      signIn: 'Logga in',
      signUp: 'Registrera',
      needAccount: 'Behöver du ett konto? Registrera dig',
      haveAccount: 'Har du ett konto? Logga in',
    },
    errors: {
      invalidCredentials: 'Ogiltig e-post eller lösenord.',
      emailAlreadyUsed: 'Den här e-posten är redan registrerad. Prova att logga in.',
      weakPassword: 'Lösenordet måste vara minst 6 tecken.',
      invalidEmail: 'Ange en giltig e-postadress.',
      tooManyRequests: 'För många försök. Vänta en stund och prova igen.',
      network: 'Nätverksfel. Kontrollera anslutningen och försök igen.',
      profileSetupFailed:
        'Ditt konto skapades, men vi kunde inte slutföra spelarprofilen. Försök igen.',
      generic: 'Något gick fel.',
    },
  },
  courses: {
    heading: 'Banor',
    freshOption: 'Ny bana',
    admin: {
      badge: 'Admin',
      title: 'Firebase custom claim admin',
    },
    aria: {
      chooseCourse: 'Välj bana',
      chooseCourseAndLayout: 'Välj bana och layout',
      filterCourses: 'Filtrera banor',
      locationTools: 'Platsverktyg',
      courseCity: 'Banstad',
      newCourseCity: 'Ny banstad',
      layoutName: 'Layoutnamn',
      holePar: 'Par för hål {{hole}}',
      holeLength: 'Längd i meter för hål {{hole}}',
    },
    filters: {
      searchByName: 'Sök efter namn',
      namePlaceholder: 'Maple Hill',
      filterByCity: 'Filtrera efter stad',
      cityPlaceholder: 'Leicester',
      sortByNearest: 'Sortera närmast först',
      nearMeOnly: 'Bara nära mig',
    },
    location: {
      useMyLocation: 'Använd min plats',
      clearLocation: 'Rensa plats',
      locating: 'Lokaliserar…',
      requesting: 'Hämtar din plats…',
      usingLocation: 'Använder din plats ({{latitude}}, {{longitude}}).',
      deniedStatus: 'Platsbehörighet nekad. Du kan fortfarande söka på namn och stad.',
      unavailableStatus: 'Kunde inte avgöra din plats.',
      unsupported: 'Geolokalisering stöds inte i den här webbläsaren.',
      permissionDenied: 'Behörighet nekad. Aktivera platsbehörighet för att sortera efter avstånd.',
      unavailableNow: 'Kunde inte avgöra din plats just nu.',
    },
    empty: {
      noCourses: 'Inga banor ännu. Lägg till en i banpanelen (under listan på små skärmar).',
      noMatches: 'Inga banor matchar dina filter ännu.',
      noLayouts: 'Den här banan har ingen sparad layout ännu.',
      detailsTitle: 'Baninställningar',
      detailsIntro: 'Skapa en bana här och fyll i par och längd för varje hål i samma panel.',
    },
    courseCard: {
      fallbackMeta: 'Baninformation',
      distanceKmAway: '{{distanceKm}} km bort',
    },
    layoutsForCourse: 'Layouter för {{courseName}}',
    courseDetailsFor: 'Bana · {{courseName}}',
    forms: {
      courseName: 'Bannamn',
      courseNamePlaceholder: 'Bannamn',
      cityPlaceholder: 'Stad',
      cityOptionalPlaceholder: 'Stad (valfritt)',
      addTemplate: 'Lägg till mall',
      editSelectedTemplate: 'Redigera vald mall',
      newCourse: 'Ny bana',
      courseHoleCount: 'Hål',
      nineHoles: '9 hål',
      eighteenHoles: '18 hål',
      layoutName: 'Layoutnamn',
      templateHoleDetails: 'Par och längd per hål',
      holeNumber: 'Hål',
      holePar: 'Par',
      holeLength: 'Längd (m)',
      holeLengthPlaceholder: 'Valfritt',
    },
    actions: {
      saving: 'Sparar…',
      saveName: 'Spara namn',
      addTemplate: 'Lägg till mall',
      saveTemplate: 'Spara mall',
      add: 'Lägg till',
      back: '← Tillbaka till banor',
    },
    templateMeta: {
      holeCount_one: '{{count}} hål',
      holeCount_other: '{{count}} hål',
      default: 'standard',
    },
    hints: {
      onlyAdminsRename: 'Endast administratörer kan byta namn på kanoniska banor.',
      wholeCourseDeleteAdminOnly:
        'Att ta bort en hel bana kräver administratörskonto. Be en administratör om du behöver radera en bana.',
      nonStandardHoleCount:
        'Den här layouten har {{count}} hål. Du kan fortfarande ändra par och längd; byte mellan 9 och 18 erbjuds bara för standardlayouter med 9 eller 18 hål.',
      templateReadOnly: 'Bara den som lade till layouten eller en administratör kan ändra par och längd här.',
      singleLayoutAutoUsed:
        'Den här banan har en sparad layout; den används automatiskt. Du behöver inte välja mall.',
      multipleLayoutsUsingDefault:
        'Den här banan har flera sparade layouter. Appen använder layouten markerad som standard, annars den första alfabetiskt efter namn om ingen är standard.',
    },
    selection: {
      selected: 'Vald',
      active: '{{courseName}} · {{holeCount}} hål',
      templateId: '(mall-id {{templateId}})',
    },
    favourites: {
      addAria: 'Lägg till {{courseName}} i favoriter',
      removeAria: 'Ta bort {{courseName}} från favoriter',
    },
    errors: {
      courseNameRequired: 'Bannamn krävs.',
      createCourseFailed: 'Kunde inte skapa bana.',
      renameCourseFailed: 'Kunde inte byta namn på bana.',
      createTemplateFailed: 'Kunde inte skapa mall.',
      updateTemplateFailed: 'Kunde inte uppdatera mall.',
      favoriteSyncFailed: 'Kunde inte läsa in favoritbanor.',
      favoriteUpdateFailed: 'Kunde inte uppdatera favoritbana.',
    },
    deleteCourseLabel: 'Ta bort bana',
    deleteCourse: 'Ta bort bana',
    deletingCourse: 'Tar bort…',
    deleteCourseConfirm:
      'Ta bort "{{courseName}}" och alla tillhorande layouter? Det går inte att angra.',
    deleteCourseError: 'Kunde inte ta bort banan',
    highscores: {
      title: 'Topplista',
      empty: 'Inga avslutade rundor på den här banan ännu.',
      loadError: 'Kunde inte hämta topplistan.',
      strokesLabel_one: '{{count}} slag',
      strokesLabel_other: '{{count}} slag',
      deltaToPar: '{{delta}} mot par',
      unknownPlayer: 'Okänd spelare',
    },
    overview: {
      title: 'Översikt',
      totalLength: 'Total längd',
      totalLengthValue: '{{meters}} m',
      bestScore: 'Bäst',
      averageScore: 'Snitt',
    },
    stats: {
      ariaSummary: 'Banstatistik',
      totalRounds: 'Totalt antal rundor',
      uniquePlayers: 'Unika spelare',
      totalThrows: 'Totalt antal kast',
      averageNet: 'Snitt mot par',
    },
    detail: {
      notFoundTitle: 'Banan hittades inte',
      notFound: 'Den här banan är inte tillgänglig längre.',
    },
  },
  scoring: {
    title: 'Rundor och poäng',
    tabs: {
      scorecard: 'Scorecard',
      results: 'Resultat',
      participants: 'Deltagare',
    },
    sections: {
      startRound: 'Starta en runda',
      yourRounds: 'Dina rundor',
      holeByHole: 'Hål-för-hål-poäng',
      results: 'Rundresultat',
      roundParticipants: 'Rundans deltagare',
    },
    buttons: {
      savedCourse: 'Sparad bana',
      freshInstance: 'Ny instans',
      newRound: 'Ny runda',
      addParticipants: 'Lägg till deltagare',
      addAnonymous: 'Spelare utan konto',
      addPlayer: 'Lägg till spelare',
      removeAnonymous: 'Ta bort',
      select: 'Välj',
      selected: 'Vald',
      delete: 'Ta bort',
      completeRound: 'Avsluta runda',
      completeRoundHint: 'Det här avslutar hela rundan för alla — inte bara hålet du tittar på.',
      completeRoundOwnerOnly: 'Bara spelaren som startade rundan kan markera den som avslutad.',
      saveLastHole: 'Spara poäng',
      retryPromotion: 'Försök igen',
      retrySave: 'Försök spara igen',
      removeParticipant: 'Ta bort från runda',
      replaceParticipant: 'Byt ut…',
      confirmReplace: 'Byt deltagare',
      cancelReplace: 'Avbryt',
    },
    labels: {
      addAnonymous: 'Spelare utan konto',
      playerNameOptional: 'Spelarnamn (valfritt)',
      anonymousParticipant: 'Anonym deltagare',
    },
    placeholders: {
      anonymousName: 'Ange spelarens namn',
      playerName: 'Ange spelarens namn',
    },
    messages: {
      anonymousNameRequired: 'Spelarnamn krävs.',
      anonymousNameTooLong: 'Spelarnamn får vara högst {{max}} tecken.',
      participantAdded: 'Lade till 1 deltagare.',
      participantsAdded: 'Lade till {{count}} deltagare.',
      participantRemoved: 'Tog bort 1 deltagare från den här rundan.',
      participantReplaced: 'Bytte ut en spelare på den här rundan. Befintliga hålpoäng flyttas till den nya spelaren.',
    },
    start: {
      courseToPlay: 'Bana att spela',
      savedSelection:
        'Använder {{courseName}}. Par och längd hämtas från den sparade layouten för den rundlängd du väljer.',
      defaultTemplateLabel: 'standardmall',
      noSavedCourses: 'Inga sparade banor tillgängliga. Välj Ny bana eller lägg till en bana först.',
      freshHint: 'Starta snabbt med namn och antal hål, fyll sedan i par/längd medan du spelar.',
      courseName: 'Bannamn',
      courseNamePlaceholder: 'Ange bannamn',
      roundLength: 'Rundlängd',
      holes9: '9 hål',
      holes18: '18 hål',
      holesFullLayout: 'Hel layout ({{count}} hål)',
      holes: 'Hål',
      participants: 'Deltagare',
      searchParticipantsPlaceholder: 'Sök spelare via namn',
      visibility: 'Synlighet',
    },
    visibility: {
      private: 'Privat',
      unlisted: 'Olistad',
      public: 'Offentlig',
    },
    rounds: {
      none: 'Inga rundor ännu.',
      completed: 'klar',
      freshDraft: 'nytt utkast · {{name}}',
      unnamed: 'utan namn',
      savedCourse: 'Sparad layout · {{holeCount}} hål · bana {{courseId}}',
      summary: '{{totalStrokes}}/{{totalPar}} ({{totalDelta}}) · {{scoredHoles}}/{{holeCount}} hål',
      latestScoreAria: '{{label}}: {{strokes}} kast på par {{par}}',
      latestScoreTitle: '{{label}} ({{delta}} mot par)',
      listLeaderAria: 'Rundledare: {{names}}, {{delta}} mot par',
      listLeaderDelta: '{{delta}} mot par',
      noScores: 'inga poäng',
      roundTotal:
        'Rundtotal {{totalStrokes}}/{{totalPar}} ({{totalDelta}}) · {{fullyScoredHoles}}/{{holeCount}} hål klara',
      selectRoundToLoadHoleForm: 'Välj en runda för att ladda hålformuläret.',
      selectRoundToRecordScores: 'Välj en runda för att registrera hålpoäng.',
      disclosureSummary: 'Dina rundor ({{count}})',
      showAllRounds: 'Visa alla {{count}} rundor',
      showFewerRounds: 'Visa färre rundor',
    },
    participants: {
      selectRoundFirst: 'Välj en runda först.',
      playerSummary: '{{totalStrokes}}/{{totalPar}} ({{totalDelta}}) · {{scoredHoles}} hål',
      grandTotal_one: 'Totalsumma: {{totalStrokes}}/{{totalPar}} ({{totalDelta}}) över {{participantCount}} deltagare.',
      grandTotal_other:
        'Totalsumma: {{totalStrokes}}/{{totalPar}} ({{totalDelta}}) över {{participantCount}} deltagare.',
      addParticipants: 'Lägg till deltagare',
      teams: 'Lag',
      teamLine: '{{teamName}}: {{members}}',
      searchUsersPlaceholder: 'Sök spelare via namn',
      addSelectedParticipants: 'Lägg till spelare',
      replaceIntro:
        'Välj ett registrerat konto som tar platsen. Redan inlagda hålpoäng flyttas till den nya spelaren.',
      replaceSearchLabel: 'Sök ersättningsspelare',
      replaceSearchPlaceholder: 'Sök via visningsnamn',
    },
    stepper: {
      navigateHoleGroupAria: 'Hålnavigation',
      previousHoleAria: 'Föregående hål',
      nextHoleAria: 'Nästa hål',
      currentHoleCaption: 'Hål',
      currentHoleInputAria: 'Aktuellt hål, 1–{{total}}',
      honorThrowFirst: '{{names}} honör',
      nextHoleCta: 'Nästa hål',
    },
    holeForm: {
      sectionAria: 'Formulär för hål {{holeNumber}}',
      par: 'Par',
      parAria: 'Par för hål {{holeNumber}}',
      lengthMeters: 'Längd (m)',
      lengthAria: 'Hålets längd i meter för hål {{holeNumber}}',
    },
    form: {
      savedLayoutLockedHint:
        'Par och längd är låsta till banans layout. Rundägare eller administratör kan låsa upp par för att rätta fel när det redan finns poäng.',
    },
    teamRows: {
      listAria: 'Lagets poängrader',
      strokesForTeamAria: 'Kast för {{teamName}}',
    },
    playerRows: {
      listAria: 'Spelarens poängrader',
      strokesSummary: '{{strokes}} kast',
      noScoreYet: 'Ingen poäng ännu',
      strokes: 'Kast',
      strokesForPlayerAria: 'Kast för {{displayName}}',
      notationAria: '{{displayName}}: {{label}}',
      enterStrokesHint: 'Ange kast för att se notation.',
      needParForResult: 'Ange hålets par för att se resultat mot par',
    },
    results: {
      winnerSingle: 'Vinnare: {{name}}',
      winnersTied: 'Vinnare: {{names}}',
      standingsAria: 'Slutresultat',
      resultLine: '{{name}} - {{strokes}} - ({{toPar}})',
      placeFirst: '1:a plats',
      placeSecond: '2:a plats',
      placeThird: '3:e plats',
      placeNumber: 'Plats {{place}}',
    },
    summary: {
      caption: 'Sparade kast per hål (scrolla åt sidan på små skärmar).',
      aria: 'Sammanfattning av rundan per hål',
      holeCol: 'Hål',
      playerCol: 'Spelare',
      teamCol: 'Lag',
      totalAbbr: 'Tot',
      parRow: 'Par',
      lengthCol: 'm',
      cellTitle: '{{strokes}} kast, par {{par}}: {{label}} ({{delta}})',
    },
    saveState: {
      unsavedChanges: 'Osparade ändringar',
      saving: 'Sparar…',
      saveFailed: 'Sparning misslyckades',
      saved: 'Sparad',
    },
    scoreTier: {
      albatrossPlus: 'Albatross eller bättre',
      eagle: 'Eagle',
      birdie: 'Birdie',
      par: 'Par',
      bogey: 'Bogey',
      doubleBogey: 'Dubbelbogey',
      tripleBogeyPlus: 'Trippelbogey eller sämre',
    },
    notices: {
      roundCreated: 'Rundan skapades. Börja poängsätta hål för hål.',
      addedParticipants_one: 'Lade till {{count}} deltagare.',
      addedParticipants_other: 'Lade till {{count}} deltagare.',
      roundDeleted: 'Rundan togs bort.',
      roundCompletedPromoted: 'Rundan avslutades. Ny bana har flyttats till sparad ban-katalog.',
      roundCompletedPromotionPending: 'Rundan avslutades. Flytten väntar och behöver försökas igen när du är online.',
      promotionSucceeded: 'Flytt lyckades. Ny bana finns nu i sparade banor.',
      promotionStillPending: 'Väntar fortfarande. Försök igen när nätverket fungerar.',
    },
    confirmations: {
      deleteRound: 'Ta bort den här rundan permanent? Detta kan inte ångras.',
    },
    fields: {
      par: 'par',
      length: 'längd',
    },
    errors: {
      freshHoleMetadataIncomplete: 'Ny hålmetadata är ofullständig.',
      holeIssue: 'Hål {{holeNumber}}: {{fields}}',
      failedToAutosaveHole: 'Kunde inte autospara hål.',
      selectCourseOrFresh: 'Välj en sparad bana eller använd Ny bana.',
      selectedCourseHasNoTemplates: 'Den valda banan har inga mallar att starta från.',
      selectTemplateOrFresh: 'Välj en sparad banmall eller byt till ny instans.',
      failedToCreateRound: 'Kunde inte skapa runda.',
      failedToAddParticipant: 'Kunde inte lägga till deltagare.',
      failedToRemoveParticipant: 'Kunde inte ta bort deltagare.',
      failedToReplaceParticipant: 'Kunde inte byta ut deltagare.',
      failedToDeleteRound: 'Kunde inte ta bort runda.',
      roundCannotCompleteWithDetails:
        'Rundan kan inte slutföras ännu. Fyll i saknad hålmetadata först. {{details}}',
      failedToCompleteRound: 'Kunde inte slutföra runda.',
      promotionBlockedWithDetails: 'Flytten blockeras fortfarande av saknad hålmetadata. {{details}}',
      failedToRetryPromotion: 'Kunde inte försöka flytt igen.',
      roundNotFound: 'Rundan hittades inte.',
      notRoundParticipant: 'Du är inte deltagare i den här rundan.',
      notPermittedToManageRoundRoster: 'Du får inte ändra spellistan för den här rundan.',
      cannotRemoveRoundOwner: 'Rundvärd kan inte tas bort från spellistan.',
      cannotReplaceRoundOwner: 'Rundvärd kan inte bytas ut på spellistan.',
      replacementAlreadyParticipant: 'Den spelaren ingår redan i rundan.',
      replacementMustBeRegistered: 'Ersättning måste vara ett registrerat konto, inte en gäst.',
      replaceSourceEqualsTarget: 'Välj en annan spelare än den du byter ut.',
      replacementUserRequired: 'Välj en ersättningsspelare.',
      targetNotRoundParticipant: 'Vald deltagare finns inte i den här rundan.',
      roundDoesNotSupportFreshMetadataEdits: 'Den här rundan stöder inte redigering av ny hålmetadata.',
      setHoleCountAndFillMetadata: 'Ange antal hål och fyll i saknad hålmetadata innan rundan slutförs.',
      parRequiredBeforeComplete: 'Par krävs innan rundan kan slutföras.',
      parRange: 'Par måste vara ett heltal i intervallet {{min}}-{{max}}.',
      lengthRequiredBeforeComplete: 'Längd krävs innan rundan kan slutföras.',
      lengthRangeMeters: 'Längd måste vara ett heltal i intervallet {{min}}-{{max}} meter.',
      freshRoundHoleCountRange: 'Nya rundor måste innehålla {{min}}-{{max}} hål.',
      holeRange: 'Hål måste vara ett heltal i intervallet {{min}}-{{max}}.',
      holeSequentialExpected: 'Hålnumreringen måste vara sekventiell (förväntade {{expected}}).',
    },
    aria: {
      workspaceTabs: 'Flikar för poängarbetsyta',
      selectRoundParticipants: 'Välj rundans deltagare',
      inviteParticipants: 'Bjud in deltagare',
      replaceParticipantPanel: 'Byt ut runddeltagare',
      replaceParticipantChoices: 'Välj ersättningsspelare',
      totalRoundDelta: 'Total runddifferens {{delta}}',
    },
    legend: 'Legend: Eagle+ <= -2, Birdie -1, Par 0, Bogey +1, Double+ >= +2.',
  },
  dashboard: {
    title: 'Din översikt',
    publicTitle: '{{name}} översikt',
    statsAria: 'Rundstatistik',
    courseFilterLabel: 'Bana',
    courseFilterAria: 'Filtrera statistik per bana',
    courseFilterAll: 'Alla banor',
    bestVsPar: 'Bästa mot par',
    avgVsPar: 'Snitt mot par',
    roundsPlayed: 'Spelade rundor',
    totalThrows: 'Totalt antal kast',
    totalMeters: 'Spelade meter',
    wins: 'Vinster',
    ongoingRounds: 'Pågående rundor',
    ongoingBadge: 'Pågår',
    trendCaption: 'Slag mot par per runda',
    trendTooltipNoDate: 'Inget datum',
    trendAria: 'Diagram över slag mot par för avslutade rundor',
    noCompletedRounds: 'Slutför en runda för att se trenden här.',
    startNewRound: 'Starta ny runda',
    recentRounds: 'Senaste rundor',
    viewAllRounds: 'Alla rundor',
    noRoundsYet: 'Inga rundor ännu. Starta en med knappen ovan.',
    browseCourses: 'Utforska banor',
    headToHeadTitle: 'Du mot {{name}}',
    headToHeadSectionAria: 'Gemensamma avslutade rundor mot {{name}}',
    headToHeadStatsAria: 'Vinster, oavgjorda och förluster utifrån ditt perspektiv',
    headToHeadWins: 'Dina vinster',
    headToHeadTies: 'Oavgjort',
    headToHeadLosses: 'Dina förluster',
    headToHeadEmpty: 'Inga avslutade rundor tillsammans ännu.',
    headToHeadNoComparable:
      'Ni har avslutat rundor tillsammans, men inga gick att jämföra än (ofullständiga eller olika scorekort).',
    headToHeadCompared: 'Baserat på {{count}} jämförbara rundor.',
    headToHeadSkipped: '{{count}} rundor hoppades över (ofullständiga eller olika scorekort).',
  },
  rounds: {
    new: {
      title: 'Ny runda',
      visibilityPublicHint: 'Nya rundor listas på spelarnas översikter för synlighet.',
      continueToScorecard: 'Fortsätt till scorekort',
      wizard: {
        steps: {
          course: 'Bana',
          players: 'Spelare',
          review: 'Granska',
        },
        format: {
          title: 'Hur poängsätter ni?',
          individualTitle: 'Vanlig runda',
          individualHint: 'Varje spelare fyller i egna poäng på varje hål.',
          scrambleTitle: 'Scramble',
          scrambleHint: 'Ett lagpoäng per hål. Tilldela spelare till lag nedan.',
          savedTeamsTitle: 'Dina sparade lag',
          savedTeamsHint: 'Dessa förval sparas på din profil. Använd dem i rundan och justera vid behov.',
          savedTeamsAria: 'Sparade scramble-lag',
          savedTeamLine: '{{teamName}}: {{members}}',
          savedTeamNoMembers: 'Inga medlemmar ännu',
          applySavedTeams: 'Använd sparade lag',
          useSavedTeam: 'Använd lag',
          teamsDisclosureSummary: 'Ställ in lag',
          unknownPlayer: 'Okänd spelare',
          teamsEditorTitle: 'Lag i den här rundan',
          teamsEditorHint: 'Välj vem som spelar tillsammans. Alla i startlistan kan vara med i ett lag.',
        },
        courseMode: {
          saved: 'Sparad bana',
          quick: 'Ny bana',
        },
        courseSearchPlaceholder: 'Sök banor efter namn',
        courseListAria: 'Välj en sparad bana',
        favoriteBadge: 'Favorit',
        quickHint: 'Starta snabbt med namn och antal hål, fyll i par och längd under spelet.',
        roster: {
          title: 'Vilka spelar',
          you: 'Du',
          guestBadge: 'Gäst',
        },
        guestNameLabel: 'Gästnamn',
        guestNamePlaceholder: 'Namn',
        addGuest: 'Lägg till gäst',
        review: {
          course: 'Bana',
          players: 'Spelare',
          format: 'Format',
          formatIndividual: 'Vanlig runda',
          formatScramble: 'Scramble',
          teams: 'Lag',
          teamLine: '{{teamName}}: {{members}}',
          savedCourse: '{{courseName}}',
          quickCourse: '{{courseName}} · {{holeCount}} hål',
        },
        nav: {
          next: 'Nästa',
          back: 'Tillbaka',
        },
      },
    },
    list: {
      title: 'Dina rundor',
      empty: 'Inga rundor ännu.',
      countLabel: '{{count}} rundor',
    },
    scorecard: {
      backHome: '← Tillbaka till översikten',
      backToPlayer: '← Tillbaka till spelaren',
      backToPlayerNamed: '← Tillbaka till {{name}}',
      notFoundOrNoAccess: 'Rundan finns inte. Den kan ha tagits bort eller så är du inte deltagare.',
      readOnlyTitle: 'Scorekort · {{courseName}}',
      readOnlyMetaStarted: 'Startade {{startedAt}}',
      readOnlyMetaCompleted: 'Startade {{startedAt}} · Slutförd {{completedAt}}',
      readOnlyTotalsAria: 'Spelartotaler för rundan',
    },
  },
  players: {
    intro: 'Sök i katalogen och öppna en spelare för att se deras offentliga översikt.',
    actions: {
      back: '← Tillbaka till spelare',
    },
  },
  follow: {
    title: 'Upptäck spelare',
    relationshipCounts: 'Följer {{followingCount}} · Följare {{followerCount}}',
    searchLabel: 'Sök spelare att följa',
    searchPlaceholder: 'Sök på visningsnamn',
    noResults: 'Inga användare matchar sökningen ännu.',
    errors: {
      updateRelationshipFallback: 'Kunde inte uppdatera följer-relationen.',
    },
    buttons: {
      saving: 'Sparar…',
      follow: 'Följ',
      unfollow: 'Sluta följa',
    },
  },
  social: {
    title: 'Socialt nätverk MVP',
    youFallback: 'Du',
    counts: {
      following: 'Följer {{count}}',
      followers: 'Följare {{count}}',
      directoryPlayers: 'Spelare i katalog: {{count}}',
    },
    followingListLabel: 'Följerlista:',
    emptyFollowing: 'Du följer ingen ännu.',
    search: {
      label: 'Sök spelare att följa',
      placeholder: 'Sök via visningsnamn',
    },
    actions: {
      saving: 'Sparar…',
      follow: 'Följ',
      unfollow: 'Avfölj',
      back: '← Tillbaka till spelare',
    },
    errors: {
      updateFollowRelationship: 'Kunde inte uppdatera följrelation.',
      cannotFollowSelf: 'Du kan inte följa dig själv.',
      invalidFollowRequest: 'Ogiltig följbegäran.',
    },
  },
} as const
